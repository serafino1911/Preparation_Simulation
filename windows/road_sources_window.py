"""
Finestra per la configurazione delle sorgenti stradali (Road Sources) in CALPUFF
"""
...
class RoadSourcesWindow:
    """Finestra per gestire le sorgenti stradali e i file ROAD_NAMES"""
    
    def __init__(self, parent, temp_dir):
        self.parent = parent
        self.temp_dir = temp_dir
        self.window = tk.Toplevel(parent)
        self.window.title("Configurazione Sorgenti Stradali")
        self.window.geometry("900x700")
        
        # Database delle sorgenti stradali (salvato separatamente)
        self.sources_db_file = Path("saved_configurations") / "road_sources_database.json"
    ...
    def load_sources_database(self):
        ...
        if self.sources_db_file.exists():
            try:
                ...
                print(f"Errore caricamento database sorgenti stradali: {e}")
                return {}
        return {}
    ...
    def save_sources_database(self):
        ...
        try:
            ...
            print(f"Errore salvataggio database sorgenti stradali: {e}")
    ...
    def setup_ui(self):
        ...
        # ===== SEZIONE FILE ROAD_NAMES =====
        files_frame = ttk.LabelFrame(main_frame, text="File ROAD_NAMES", padding="10")
        ...
        # ===== SEZIONE SORGENTI =====
        sources_frame = ttk.LabelFrame(main_frame, text="Sorgenti Stradali", padding="10")
    ...
    def refresh_sources_list(self):
        ...
        for source in self.current_sources:
            ...
            height = source.get('Effect_height', 0)
            num_points = len(source.get('position_xyz', []))
    ...
    def add_file(self):
        ...
        dialog.title("Aggiungi File Road")
    ...
    def add_source(self):
        ...
        RoadSourceEditor(self.window, self, None)
    ...
    def edit_source(self):
        ...
        RoadSourceEditor(self.window, self, source, idx)
    ...
    def save_and_close(self):
        ...
        messagebox.showinfo("Successo", "Configurazione sorgenti stradali salvata!")
        self.window.destroy()


class RoadSourceEditor:
    """Editor per singola sorgente stradale"""
    ...
    def __init__(self, parent, main_window, source_data=None, source_idx=None):
        ...
        if source_data:
            ...
            self.effect_height = tk.DoubleVar(value=source_data.get('Effect_height', 9.0))
            self.initial_sigma_z = tk.DoubleVar(value=source_data.get('initial_sigma_z', 7.0))
            self.initial_sigma_y = tk.DoubleVar(value=source_data.get('initial_sigma_y', 10.0))
            self.points = source_data.get('position_xyz', [])
            self.emis_rates = source_data.get('emis_rates', [])
        else:
            self.source_name = tk.StringVar(value='Road1')
            self.effect_height = tk.DoubleVar(value=9.0)
            self.initial_sigma_z = tk.DoubleVar(value=7.0)
            self.initial_sigma_y = tk.DoubleVar(value=10.0)
            self.points = []
            self.emis_rates = []
    ...
    def setup_ui(self):
        ...
        title = "Modifica Sorgente Stradale" if self.is_edit else "Nuova Sorgente Stradale"
        ...
        ttk.Label(params_frame, text="Effective Height (m):").grid(row=1, column=0, sticky=tk.W, pady=5)
        ttk.Entry(params_frame, textvariable=self.effect_height, width=15).grid(row=1, column=1, sticky=tk.W, pady=5, padx=5)
        
        ttk.Label(params_frame, text="Initial Sigma Z:").grid(row=2, column=0, sticky=tk.W, pady=5)
        ttk.Entry(params_frame, textvariable=self.initial_sigma_z, width=15).grid(row=2, column=1, sticky=tk.W, pady=5, padx=5)
        
        ttk.Label(params_frame, text="Initial Sigma Y:").grid(row=3, column=0, sticky=tk.W, pady=5)
        ttk.Entry(params_frame, textvariable=self.initial_sigma_y, width=15).grid(row=3, column=1, sticky=tk.W, pady=5, padx=5)
        
        # === PUNTI STRADA ===
        points_frame = ttk.LabelFrame(scrollable_frame, text="Punti Strada (X, Y, Z)", padding="10")
        ...
        # Titolo mappa
        ttk.Label(right_frame, text="Percorso Strada", 
    ...
    def refresh_points_list(self):
        ...
        for i, point in enumerate(self.points):
            x, y, z = point
            self.points_listbox.insert(tk.END, f"Punto {i+1}: X={x}, Y={y}, Z={z}")
    ...
    def add_point(self):
        ...
        ttk.Label(frame, text="Z (elevazione):").grid(row=2, column=0, sticky=tk.W, pady=5, padx=(0, 10))
        z_entry = ttk.Entry(frame, textvariable=z_var, width=20)
        z_entry.grid(row=2, column=1, pady=5, padx=5, sticky=(tk.W, tk.E))
        
        ttk.Separator(frame, orient='horizontal').grid(row=3, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=10)
        ...
        def save_point():
            ...
            self.points.append([x_var.get(), y_var.get(), z_var.get()])
            self.refresh_points_list()
            self.update_map()
            dialog.destroy()
    ...
    def edit_point(self):
        ...
        z_var = tk.DoubleVar(value=point[2])
        
        ttk.Label(frame, text="X:").grid(row=0, column=0, sticky=tk.W, pady=5)
        ttk.Entry(frame, textvariable=x_var, width=20).grid(row=0, column=1, pady=5, padx=5)
        
        ttk.Label(frame, text="Y:").grid(row=1, column=0, sticky=tk.W, pady=5)
        ttk.Entry(frame, textvariable=y_var, width=20).grid(row=1, column=1, pady=5, padx=5)
        
        ttk.Label(frame, text="Z:").grid(row=2, column=0, sticky=tk.W, pady=5)
        ttk.Entry(frame, textvariable=z_var, width=20).grid(row=2, column=1, pady=5, padx=5)
        
        def save_point():
            self.points[idx] = [x_var.get(), y_var.get(), z_var.get()]
            self.refresh_points_list()
            dialog.destroy()
    ...
    def save_source(self):
        ...
        if len(self.points) < 2:
            messagebox.showerror("Errore", "Inserire almeno 2 punti per la strada")
            return
        ...
        source = {
            ...
            'Effect_height': self.effect_height.get(),
            'position_xyz': self.points,
    ...
    def update_map(self):
        ...
        if len(lat_lon_points) >= 2:
            self.road_path = self.map_widget.set_path(
                lat_lon_points,
                color="red",
                width=3
            )
